...
public class EfCoreFileRepository : IFileRepository
{
    ...
    public Task<Admin?> GetAdminAsync(string username, string pass)
    {
      var admins=  _db.Admins.Where(c => c.Username == username && c.PasswordHash == pass).ToList();
        if ( admins.Count>0)
            {
                return Task.FromResult(admins[0]);
            }
        else
            {
